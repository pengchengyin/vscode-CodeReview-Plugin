...
import { ReviewComment, ValuePair, Column, RecordColumns, CommentBody, CommitComment } from '../models/ReviewComment';
...
export class ReviewCommentManager {
    ...
    private lineMarkers: vscode.TextEditorDecorationType[] = [];
    private systemColumns: RecordColumns | null = null;

    constructor(context: vscode.ExtensionContext, configManager: ConfigManager, networkManager: NetworkManager) {
    ...
    private loadComments(): void {
        ...
        const commentsData = this.context.globalState.get<ReviewComment[]>('reviewComments', []);
    ...
    private loadSystemColumns(): void {
        // 首先尝试从全局状态加载服务端字段配置
        const savedColumns = this.context.globalState.get<RecordColumns>('systemColumns');
        if (savedColumns) {
    ...
    public getSystemColumns(): RecordColumns | null {
        return this.systemColumns;
    }

    public async saveSystemColumns(columns: RecordColumns): Promise<void> {
    ...
    public getUserListFromColumns(): Array<{value: string, label: string}> {
        ...
        console.log('getUserListFromColumns: 所有列的 columnCode:', this.systemColumns.columns.map((col: any) => col.columnCode));
        
        // 查找检视人员字段的配置
        const reviewerColumn = this.systemColumns.columns.find((col: any) => 
            col.columnCode === 'reviewer' || col.fieldName === 'reviewer' || col.fieldName === '检视人员'
        );
        ...
        if (reviewerColumn && reviewerColumn.enumValues) {
            ...
            const userList = reviewerColumn.enumValues.map((item: any) => ({
                value: item.value || item.id || item.account,
                label: item.showName || (item as any).label || (item as any).name || item.value || item.id || item.account
            }));
    ...
    private getDisplayNameForUser(userValue: string): string | null {
        ...
        if (reviewerColumn && reviewerColumn.enumValues) {
            const userItem = reviewerColumn.enumValues.find((item: any) => 
                item.value === userValue || item.id === userValue || item.account === userValue
            );
            if (userItem) {
                return userItem.showName || (userItem as any).label || (userItem as any).name || userItem.value;
            }
        }
    ...
    public async addComment(comment: ReviewComment): Promise<void> {
    ...
    public async updateComment(id: string, updates: Partial<ReviewComment>): Promise<void> {
    ...
    public async deleteComments(ids: string[]): Promise<void> {
    ...
    public getCommentById(id: string): ReviewComment | undefined {
    ...
    public async commitToServer(projectId?: number): Promise<void> {
    ...
    public async pullFromServer(projectId: number, type: string): Promise<void> {
        if (!this.configManager.isNetworkMode()) {
            throw new Error('当前不是网络模式');
        }
    ...
    private buildCommitComment(): CommitComment {
        ...
        const comments = this.comments
            ...
            .map(comment => {
                ...
                if (comment.propValues) {
                    ...
                    (commentBody.values as any)['identifier'] = {
                        value: comment.id,
                        showName: comment.id
                    };
                    
                    // 确保 fileSnapshot 字段存在
                    (commentBody.values as any)['fileSnapshot'] = {
                    ...
                    fields.forEach(field => {
                        ...
                        const value = (comment as any)[field];
                        if (value !== undefined && value !== null) {
                            ...
                            if (field === 'projectId') {
                                ...
                                if (!isNaN(numValue)) {
                                    ...
                                    const projectName = projectMap[String(numValue) as keyof typeof projectMap] || String(numValue);
                                    (commentBody.values as any)[field] = {
                                ...
                                if (field === 'type') {
                                    ...
                                    const typeName = typeMap[String(value) as keyof typeof typeMap] || String(value);
                                    (commentBody.values as any)[field] = {
                                        value: String(value),
                                        showName: typeName
                                    };
                ...
                console.log('buildCommitComment: identifier 字段:', (commentBody.values as any)['identifier']);
                console.log('buildCommitComment: fileSnapshot 字段:', (commentBody.values as any)['fileSnapshot']);
                return commentBody;
            });
    ...
    private convertServerComment(commentBody: CommentBody): ReviewComment {
        ...
        if (commentBody.values) {
            if (commentBody.values instanceof Map) {
                ...
                commentBody.values.forEach((valuePair, key) => {
                    (comment as any)[key] = valuePair.value;
                });
            } else {
                // 如果是普通对象
                Object.entries(commentBody.values).forEach(([key, valuePair]) => {
                    (comment as any)[key] = (valuePair as any).value;
                });
            }
        }
    ...
    public refreshLineMarkers(): void {
        ...
        fileComments.forEach((comment, index) => {
            ...
            // 创建更详细的悬停消息
            const typeMap = { '1': '问题', '2': '建议', '3': '疑问' };
            const typeText = typeMap[comment.type as keyof typeof typeMap] || comment.type;
            
            const confirmResultMap = { 'unconfirmed': '未确认', 'confirmed': '已确认', 'rejected': '已拒绝' };
            const confirmStatus = confirmResultMap[comment.confirmResult as keyof typeof confirmResultMap] || '未确认';
    ...
    public async jumpToComment(comment: ReviewComment): Promise<void> {
            ...
            if (!path.isAbsolute(filePath)) {
                ...
                if (!found) {
                    ...
                    try {
                        ...
                        const similarFiles = files.filter((f: string) => f.includes(fileName) || fileName.includes(f));
