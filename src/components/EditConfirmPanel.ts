...
export class EditConfirmPanel {
    public static currentPanel: EditConfirmPanel | undefined;
    public static readonly viewType = 'editConfirmPanel';
    private readonly _panel: vscode.WebviewPanel;
    private readonly _extensionUri: vscode.Uri;
    private _disposables: vscode.Disposable[] = [];
    private _comment: ReviewComment = {} as ReviewComment;
    private _onSaveCallback?: (updatedComment: ReviewComment) => void;
    private _commentManager?: any;

    public static createOrShow(extensionUri: vscode.Uri, comment: ReviewComment, onSave?: (updatedComment: ReviewComment) => void, commentManager?: any) {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, comment?: ReviewComment, onSave?: (updatedComment: ReviewComment) => void, commentManager?: any) {
    ...
    private async _updateContent(comment: ReviewComment, onSave?: (updatedComment: ReviewComment) => void) {
    ...
    private async _getHtmlForWebview(comment: ReviewComment): Promise<string> {
...
<body>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="assignConfirmer" 
                        ...
                        ${userList.map(user => 
                            `<option value="${user.value}" ${comment.assignConfirmer === user.value ? 'selected' : ''}>${user.label}</option>`
                        ).join('')}
                    </select>
            ...
            <div class="form-group">
                ...
                <select id="confirmResult" name="confirmResult" class="form-select" required>
                    ${confirmResultOptions.map(option => 
                        `<option value="${option.value}" ${comment.confirmResult === option.value ? 'selected' : ''}>${option.label}</option>`
                    ).join('')}
                </select>
    ...
    private async _getUserList(): Promise<Array<{value: string, label: string}>> {
