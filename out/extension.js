...
const ReviewCommentManager_1 = require("./managers/ReviewCommentManager");
...
const CommentDetailPanel_1 = require("./components/CommentDetailPanel");
const FileSnapshotPanel_1 = require("./components/FileSnapshotPanel");
const EditConfirmPanel_1 = require("./components/EditConfirmPanel");
function activate(context) {
    ...
    const commentManager = new ReviewCommentManager_1.ReviewCommentManager(context, configManager, networkManager);
    // 注册数据提供者
    const provider = new CodeReviewProvider_1.CodeReviewProvider(commentManager);
    const providerDisposable = vscode.window.registerTreeDataProvider('codeReviewPanel', provider);
    // 注册命令
    const addCommentCommand = new AddCommentCommand_1.AddCommentCommand(commentManager);
    const settingsCommand = new SettingsCommand_1.SettingsCommand(configManager);
    ...
    const jumpToCommentDisposable = vscode.commands.registerCommand('codeReviewHelper.jumpToComment', async (commentItem) => {
    ...
    const clearCommentsDisposable = vscode.commands.registerCommand('codeReviewHelper.clearComments', async () => {
        const result = await vscode.window.showWarningMessage('确定要清空所有评审意见吗？此操作不可恢复！', '确定', '取消');
        if (result === '确定') {
    ...
    const commitToServerDisposable = vscode.commands.registerCommand('codeReviewHelper.commitToServer', async () => {
        try {
            ...
            if (!configManager.isNetworkMode()) {
                const enableNetwork = await vscode.window.showWarningMessage('当前不是网络模式，是否要启用网络模式？', '启用网络模式', '取消');
            ...
            // 显示确认对话框
            const confirm = await vscode.window.showWarningMessage(`确定要推送 ${comments.length} 条评审意见到服务器吗？`, '确定推送', '取消');
            if (confirm !== '确定推送') {
                return;
            }
            // 显示进度
            await vscode.window.withProgress({
                ...
                await commentManager.commitToServer(selectedProject.value);
            });
    ...
    const pullFromServerDisposable = vscode.commands.registerCommand('codeReviewHelper.pullFromServer', async () => {
        try {
            ...
            await vscode.window.withProgress({
                ...
                await commentManager.pullFromServer(selectedProject.value, selectedType.value);
            });
            await provider.refresh();
    ...
    const deleteCommentDisposable = vscode.commands.registerCommand('codeReviewHelper.deleteComment', async (comment) => {
        if (!comment) {
        ...
        const result = await vscode.window.showWarningMessage(`确定要删除这条评审意见吗？\n文件: ${reviewComment.filePath}\n行号: ${reviewComment.lineRange}`, '确定删除', '取消');
        if (result === '确定删除') {
            try {
    ...
    const showCommentDetailDisposable = vscode.commands.registerCommand('codeReviewHelper.showCommentDetail', async (comment) => {
        if (!comment) {
        ...
        // 使用 WebView 面板显示详情
        CommentDetailPanel_1.CommentDetailPanel.createOrShow(context.extensionUri, reviewComment, commentManager);
    });
    const showOriginalSnapshotDisposable = vscode.commands.registerCommand('codeReviewHelper.showOriginalSnapshot', async (comment) => {
        if (!comment) {
            vscode.window.showWarningMessage('请选择要查看的评审意见');
            return;
        }
        ...
        // 使用 WebView 面板显示原始快照
        FileSnapshotPanel_1.FileSnapshotPanel.createOrShow(context.extensionUri, reviewComment);
    });
    const editConfirmDisposable = vscode.commands.registerCommand('codeReviewHelper.editConfirm', async (comment) => {
        if (!comment) {
        ...
        // 使用 WebView 面板编辑确认信息
        EditConfirmPanel_1.EditConfirmPanel.createOrShow(context.extensionUri, reviewComment, async (updatedComment) => {
            try {
    ...
    const openCommentDetailDisposable = vscode.commands.registerCommand('codeReviewHelper.openCommentDetail', async (commentId) => {
        ...
        if (comment) {
            ...
            CommentDetailPanel_1.CommentDetailPanel.createOrShow(context.extensionUri, comment, commentManager);
        }
        else {
    ...
    const refreshProjectsDisposable = vscode.commands.registerCommand('codeReviewHelper.refreshProjects', async () => {
        try {
            ...
            await vscode.window.withProgress({
                ...
                // 显示项目列表
                const projectList = projects.map((p) => `• ${p.projectName} (ID: ${p.projectId})`).join('\n');
    ...
    const debugColumnConfigDisposable = vscode.commands.registerCommand('codeReviewHelper.debugColumnConfig', async () => {
        try {
            ...
            console.log('所有列:', systemColumns.columns.map((col) => ({
                fieldName: col.fieldName,
                hasEnumValues: !!(col.enumValues && col.enumValues.length > 0),
                enumValuesCount: col.enumValues ? col.enumValues.length : 0
            })));
            // 查找检视人员字段
            const reviewerColumn = systemColumns.columns.find((col) => col.fieldName === 'reviewer' || col.fieldName === '检视人员');
            if (!reviewerColumn) {
                vscode.window.showWarningMessage('没有找到检视人员字段！');
                console.log('可用的字段名:', systemColumns.columns.map((col) => col.fieldName));
                return;
            }
    ...
    context.subscriptions.push(providerDisposable, addCommentDisposable, openPanelDisposable, settingsDisposable, jumpToCommentDisposable, exportCommentsDisposable, importCommentsDisposable, clearCommentsDisposable, commitToServerDisposable, pullFromServerDisposable, deleteCommentDisposable, showCommentDetailDisposable, showOriginalSnapshotDisposable, editConfirmDisposable, refreshDisposable, openCommentDetailDisposable, refreshProjectsDisposable, debugColumnConfigDisposable, testStorageDisposable, testPathDisposable, fileChangeDisposable, editorChangeDisposable);
    // 初始化行标记
    commentManager.refreshLineMarkers();
}
