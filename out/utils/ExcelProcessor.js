...
class ExcelProcessor {
    async exportComments(comments, filePath) {
        // 创建工作簿
    ...
    getConfirmResultValue(resultText) {
        const resultMap = {
            '未确认': 'unconfirmed',
            '已修改': '2',
            '待修改': '3',
    ...
    parseStartLine(lineRange) {
        if (!lineRange)
            return 0;
        const match = lineRange.match(/(\d+)\s*~\s*(\d+)/);
        return match ? parseInt(match[1]) : 0;
    }
    parseEndLine(lineRange) {
        if (!lineRange)
            return 0;
        const match = lineRange.match(/(\d+)\s*~\s*(\d+)/);
