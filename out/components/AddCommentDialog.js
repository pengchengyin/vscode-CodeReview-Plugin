...
exports.AddCommentDialog = void 0;
const vscode = require("vscode");
class AddCommentDialog {
    constructor(comment, commentManager) {
    ...
    async createPanel() {
        this.panel = vscode.window.createWebviewPanel('addCommentDialog', '添加评审意见', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async getWebviewContent() {
        ...
        const typeOptionsHtml = typeOptions.map(option => `<option value="${option.value}" ${this.comment.type === option.value ? 'selected' : ''}>${option.label}</option>`).join('');
        return `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
        ...
        <form id="commentForm">
            ...
            <div class="form-group">
                ...
                <select id="assignConfirmer" name="assignConfirmer">
                    ...
                    ${userList.map(user => `<option value="${user.value}" ${this.comment.assignConfirmer === user.value ? 'selected' : ''}>${user.label}</option>`).join('')}
                </select>
            ...
            <div class="form-group">
                ...
                <select id="projectId" name="projectId">
                    ...
                    ${projectList.map(project => `<option value="${project.value}" ${this.comment.projectId === project.value ? 'selected' : ''}>${project.label}</option>`).join('')}
                </select>
...
//# sourceMappingURL=AddCommentDialog.js.map